...
\begin{column}{0.44\textwidth}
  ...
  \small Solves LP to allocate jobs across different GPU types.\\
...
\begin{column}{0.44\textwidth}
  ...
  \textcolor{cardinalred}{\textbf{Gap:} No flexible policy framework \& Ignore GPU heterogeneity.}
\end{column}
\end{columns}
...
\includegraphics[
    ...
    keepaspectratio
]{fig_literature_genealogy.png}

\vspace{4pt}

\begin{minipage}{0.95\linewidth}
\small
Research lineage of GPU cluster scheduling. 
Heterogeneity-aware systems (blue) and fragmentation-aware systems (red). 
\textbf{Our work bridges both directions.}
\end{minipage}
...
\small
\textbf{Stage 1: Gavel Allocation}

LP allocates jobs across
different GPU types based on effective throughput.

\vspace{3pt}

\textit{Decision: which GPU type?}
\end{minipage}
\end{column}
...
\begin{minipage}[t]{\linewidth}
\small
\textbf{Stage 2: FGD Placement}

Gradient-based placement minimizes
fragmentation when selecting a node.

\vspace{3pt}

\textit{Decision: which node?}
\end{minipage}
\end{column}
...
\begin{column}{0.48\textwidth}
  \centering
  \includegraphics[width=\textwidth,height=0.7\textheight,keepaspectratio]{fig4_eval_methodology.png}\\[3pt]
...
\begin{itemize}
  ...
  \item \textbf{GPU communication overhead} not modeled
  \item \textbf{No performance interference model}: co-located jobs assumed to run at constant efficiency
\end{itemize}
...
\small
Simulator: \empr{\href{https://github.com/Turquoise-T/cs244c-GPU\_fragment}{github.com/Turquoise-T/cs244c-GPU\_fragment}}\\
Viz tool: \empr{\href{https://github.com/varunr89/gpu-scheduling-viz}{github.com/varunr89/gpu-scheduling-viz}}
...
\begin{frame}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.2\linewidth]{gpu.png}
    \end{figure}
    \end{center}
\end{frame}
